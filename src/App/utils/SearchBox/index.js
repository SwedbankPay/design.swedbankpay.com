...
const SearchBox = ({ className, mobile }) => {
	...
	const results = () => {
		const tempSearchResultList = routes.map((route) =>
			route.routes.filter((val) => {
				if (searchTerm === "") {
					return "";
		...
		return (
			<ul className="item-list item-list-hover">
				{tempSearchResultList.map((directory) =>
					directory.map((result) => (
						<Link
							tabIndex="-1"
							...
							<li>
								<span
									...
									dangerouslySetInnerHTML={{
										__html: modify(result.title, searchTerm),
									}}
								></span>
		...
		<>
			{mobile ? (
				...
				>
					...
					{expanded && searchTerm !== "" && (
						<div className="result-box">{results()}</div>
					)}
				</div>
			) : (
				<div
				...
				>
					...
					{searchTerm !== "" && <div className="result-box">{results()}</div>}
				</div>
			)}
		</>
	);
