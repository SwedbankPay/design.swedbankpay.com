...
const Overview = () => (
	<section>
		...
		<p>
			Accordions can make information discovery more effective by using
			progressive disclosure, that is showing the title while hiding the
			detailed information. They can be used to shorten pages and reduce
			scrolling when content is not crucial to read in full.
		</p>
		<ComponentPreview
			language="html"
			showCasePanel
			codeFigure
			showCasePanelAdvanced={accordionShowCase}
			removeOuterTag
		/>
				...
				<div className="slab slab-plain slab-border-success pb-5">
					...
					<div className="mt-3">
						<AccordionComponent
							...
							className="show accordion-open"
							items={howToUseItems}
						/>
					</div>
				</div>
				<p>
					Body content inside the Accordion may be split into paragraphs and
					include sub-headers if needed.
				</p>
			</div>
...
const ContentGuidelines = () => (
	<section>
		...
		<ul className="list list-bullet">
			<li>
				The title in the Accordion header should be as brief as possible while
				still being clear and descriptive.
			</li>
			<li>
				The Accordion body should contain detailed information related to the
				title.
			</li>
		</ul>
	</section>
...
const DeveloperDocumentation = () => (
	<section>
		...
		<p>
			Use <CodeTags type="secondary" code="dg.accordion.init()" /> to{" "}
			<Link to="/get-started/for-developers">initialize</Link> all accordions.
			This will open and close the accordions automatically.
		</p>

		<p>
			If you want to manually open and close a specific accordion, you need to
			include an <CodeTags type="primary" code="id" /> on the{" "}
			<CodeTags type="primary" code="accordion-body" /> element.
		</p>
		<p>
			To open a given accordion, use{" "}
			<CodeTags
				type="secondary"
				code="dg.accordion.open(<accordion-body-id>)"
			/>
		</p>
		<p>
			...
			<CodeTags
				type="secondary"
				code="dg.accordion.close(<accordion-body-id>)"
			/>
		</p>
	</section>
);
	...
	return (
		<DocContainer>
			<section id="accordion-doc">
				<p className="lead">
					Accordions are components that toggles between hiding and showing
					content
				</p>
				<Overview />
				<ContentGuidelines />
