...
const Overview = () => (
	<section>
		...
		<ComponentPreview
			...
			showCasePanel
			hideCodeFigure
			showCasePanelAdvanced={toastShowCase}
		/>
		...
		<section>
			...
			<ul className="list list-bullet">
				<li>
					Toasts are temporary notifications, therefore in cases where you want
					to display a more permanent message to the user, use the{" "}
					<Link to="/components/alerts">alert</Link> component instead which
					stays permanently on the screen.
				</li>
				<li>
					Similarly, if you require action from the user before continuing to
					the next page, use a <Link to="/components/dialog">dialog</Link>{" "}
					component instead.
				</li>
			</ul>
		</section>
		...
		<section>
			...
			<p>
				The positioning of the toast component should always be at the top right
				corner of the page on desktop and top center on mobile devices.
			</p>
			...
			<p>
				The toast component should not pop up in the middle of the screen and
...
const ContentGuidelines = () => (
	<section>
		...
		<ul className="list list-bullet">
			<li>
				The toast may have a bolded title to give a quick overview of the
				notification. It’s important that the <b>message</b> reflect the toast
				type, e.g if it is an error toast, it should clearly state the issue and
				if possible explain how to solve the problem.{" "}
			</li>
		</ul>
	</section>
);
...
const DeveloperDocumentation = () => (
	<section>
		...
		<p>
			To use the Toast, call <CodeTags type="secondary" code="dg.toast()" />{" "}
		...
		<section>
			...
			<p>
				<Link to="/get-started/for-developers/">Usage with NPM</Link> will
				require you to import it from{" "}
				<CodeTags type="primary" code="@swedbankpay/design-guide" /> before
				calling it on an element.
			</p>
...
const Toast = () => (
	<DocContainer>
		<section id="toast-doc">
			<p className="lead">
				The toast component is only displayed when the user has performed an
				action. The components informs the user of the status of their request,
