...
const ShowcaseComponent = ({
	...
	return (
		<div
			className={`loader-preview-container${
				backgroundDark ? " dark" : ""
		...
		>
			{standalone ? (
				...
				<p
					className={`mt-3${backgroundDark ? " text-white" : ""}${
			...
			},
			{
				...
				content: (
					<p className=" mb-0 justify-content-center">
						<Link linkText="Click here" /> to read our term and conditions.
					</p>
				),
			},
		],
		...
		slabs: [
			{
				...
				content: (
					<p className="mb-0 justify-content-center">
						Here you can{" "}
						<Link linkText="read about the browsers we recommend" />.
					</p>
				),
			},
			{
				type: "error",
				content: (
					<Link linkText="Here you can read about the browsers we recommend" />
				),
			},
		],
...
export const overviewLinks = {
	...
	elements: [
		...
		},
		{
			...
			description:
				"Standalone links are separate to paragraph content and appear on there own. They can be user with a leading icon to further convey the result of clicking the link. Links with icons are not underlined.",
		},
	],
};
	...
	elements: [
		{
			...
			description: (
				<>
					<p>
						Actions links can be customized in a few different ways to fit
						different contexts. Default styling contains a heading and an arrow
						icon. It can be modified be adding an info text that will give more
						information or with a number badge. If a the link is directing the
						user to an external web page or opens up a new tab, this should be
						indicated using an new tab icon instead of the arrow.{" "}
					</p>
				</>
			),
		},
