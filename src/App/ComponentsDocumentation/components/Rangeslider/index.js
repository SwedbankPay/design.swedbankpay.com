...
const Overview = () => (
	<section>
		...
		<ComponentPreview
			language="html"
			showCasePanel
			showCasePanelAdvanced={sliderShowCase}
			codeFigure
		/>
		...
		<ul className="list list-bullet">
			<li>
				Use a <Link to="/components/input-field">input field</Link> instead when
				the user needs to select specific number.
			</li>
		</ul>
			...
			<React.Fragment key={i}>
				<div className={`slab slab-plain slab-border-${type} h-100 mb-2`}>
					...
					<div className="d-flex flex-column align-items-center">
						<div className="py-3 col-12 col-lg-8">{component}</div>
					</div>
				</div>
				{text}
			</React.Fragment>
...
const ContentGuidelines = () => (
	<section>
		...
		<ul className="list list-bullet">
			<li>
				The <b>label</b> should clearly explain the context of the slider
			</li>
			<li>
				Make sure to align the <b>values</b> so that they all follow the same
				structure and not mixed values or symbols
			</li>
		</ul>
	</section>
);
...
const DeveloperDocumentation = () => (
	<section>
		...
		<p className="my-3">
			Use <CodeTags type="secondary" code="dg.rangeslider.init()" /> to{" "}
			<Link to="/get-started/for-developers">initialize</Link> all rangesliders.
		</p>
		<p>
			Or{" "}
			<CodeTags type="secondary" code='dg.rangeslider.init("component-id")' />{" "}
...
const DisableState = () => (
	<section>
		...
		<p>
			Disable a slider by adding <CodeTags type="primary" code="disabled" />{" "}
			attribute to the desired slider.{" "}
		</p>
		<ComponentPreview language="html" showCasePanel codeFigure>
			<div className="d-flex flex-column align-items-center">
				<div className="py-3 col-12 col-lg-8">
					<RangesliderComponent
						id="disabled_state_example"
						label="Label"
						min={0}
						max={100}
						value={50}
...
const Rangeslider = () => {
	...
	return (
		<DocContainer>
			<p className="lead">
				The slider component lets the user increase and decrease the value by
				moving the slider along a horizontal line and lets the user choose a
				value
			</p>
			<Overview />
			<ContentGuidelines />
