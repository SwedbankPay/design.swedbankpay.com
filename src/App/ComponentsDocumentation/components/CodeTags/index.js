...
const Overview = () => (
	<section id="tag-overview">
		...
		<ComponentPreview
			language="html"
			showCasePanel
			showCasePanelAdvanced={overviewTags}
			codeFigure
		/>
		<section>
			...
			<ul>
				<li>
					...
					<Link to="/components/badge">badge</Link> or the{" "}
					<Link to="/components/status">status</Link> component instead.
				</li>
			</ul>
		</section>
		<section>
			...
			<div className="row">
				<div className="col-lg-6">
					...
					<p>
						Removable tags are related to a input component that allows to add
						and remove tags. Aim to keep the related component close so the user
				...
				<div className="col-lg-6">
					...
					<p>
						It`s recommended to add a bulk remove action next to a group with
						removable tags.
					</p>
				</div>
			</div>
...
const ContentGuidelines = () => (
	<section id="tags-content-guidelines">
		...
		<ul>
			<li>
				Keep the <b>label</b> short and concise, use 1-2 words.
			</li>
		</ul>
	</section>
);
...
const CodeTags = () => (
	<DocContainer>
		<p className="lead">
			Tags are used to label, categorize and organize items using keywords that
			describe them. Tags can for example be used in filters to represent
			selected filter values or in documentation to present inline code.
		</p>
		<Overview />
		<ContentGuidelines />
	</DocContainer>
