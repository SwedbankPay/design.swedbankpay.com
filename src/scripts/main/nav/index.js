...
class Nav {
			...
			this.submenus.forEach((submenu) => {
				...
				submenuCopy.addEventListener("click", (e) => {
					...
					if (submenu.classList.contains("submenu-open")) {
						...
						window.addEventListener("resize", this.resizeEventSubmenuOpen, {
							passive: true,
						});
	...
	_submenuCloseAll() {
		window.removeEventListener("resize", this.resizeEventSubmenuOpen, {
			passive: true,
		});
		this.submenus.forEach((submenu) =>
	...
	_hideItems() {
		const firstFour = this.listItems.filter(
			(notHidden) =>
	...
	open() {
		...
		window.addEventListener("resize", this.resizeEventMenuOpen, {
			passive: true,
		});
	}
	...
	close() {
		...
		window.removeEventListener("resize", this.resizeEventMenuOpen, {
			passive: true,
		});
	}
}
