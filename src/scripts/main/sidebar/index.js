...
class TopbarSidebar {
	...
	_contentScrollListener(id, content, headers) {
		return () => {
			...
			if (
				content.scrollTop + scrollBuffer >=
...
class Sidebar {
	...
	_popStateListener() {
		if (
			this.el &&
			!this.el
				.querySelector(".main-nav-li.active")
	...
	_activate(element) {
		if (element.parentElement.querySelector("li.active")) {
			try {
				...
				console.info("No properties to setAttribute aria-expanded");
			}
		}
	...
	_setActiveStatus(element, selector) {
		...
		[...activeElements].map((activeElement) => {
			element !== activeElement &&
				this._closeChildElements(activeElement, true);
		});
	}
...
const setActiveState = (id, group, subGroup, leaf) => {
	if (group !== null) {
		...
		leaf !== null &&
			leaf < active.querySelectorAll(SELECTORS.NAVLEAF).length &&
...
const initScrollListener = (id, contentId, headerType) => {
	...
	if (content) {
		...
		const leaves = sidebarElement
			.querySelector(`.secondary-nav-li${SELECTORS.ACTIVE}`)
...
const init = (id, topbarSidebar) => {
	if (id) {
		...
		const sidebarObject = topbarSidebar
			? _createTopbarSidebar(sidebar)
