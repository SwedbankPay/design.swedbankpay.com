import {
	extendObj,
	/* isWithinBoundingBox, */ handleScrollbar,
	openComponent,
	closeComponent,
} from "./index";
...
describe("scripts: utils", () => {
	...
	describe("- openComponent", () => {
		...
		it("prints warning message when component with provided ID does not exist", () => {
			...
			expect(console.warn).toHaveBeenCalledWith(
				'testComponent.open: No testComponent with id "invalid-id" found.'
			);
		});
		...
		it("prints a warning message when component with provided ID is already open", () => {
			...
			expect(console.warn).toHaveBeenCalledWith(
				'testComponent.open: testComponent with id "test-isOpen" is open'
			);
		});
	});
	...
	describe("- closeComponent", () => {
		...
		it("prints warning message when component with provided ID does not exist", () => {
			...
			expect(console.warn).toHaveBeenCalledWith(
				'testComponent.close: No testComponent with id "invalid-id" found.'
			);
		});
		...
		it("prints a warning message when component with provided ID is not open", () => {
			...
			expect(console.warn).toHaveBeenCalledWith(
				'testComponent.close: testComponent with id "test-isNotOpen" is not open'
			);
		});
	});
});
