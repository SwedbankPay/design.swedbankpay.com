...
describe("scripts: paginate", () => {
	...
	it("returns an object with page and current properties", () => {
		expect(paginate(10, 5)).toEqual(
			expect.arrayContaining([
				expect.objectContaining({ page: 1, current: false }),
			])
		);
	...
	it("returns the current page", () => {
		expect(paginate(10, 5)).toEqual(
			expect.arrayContaining([
				expect.objectContaining({ page: 5, current: true }),
			])
		);

		expect(paginate(100, 27)).toEqual(
			expect.arrayContaining([
				expect.objectContaining({ page: 27, current: true }),
			])
		);

		expect(paginate(1000, 666)).toEqual(
			expect.arrayContaining([
				expect.objectContaining({ page: 666, current: true }),
			])
		);
	});
	...
	it("returns correct total pages", () => {
		expect(paginate(10, 5).slice(-1)[0]).toEqual(
			expect.objectContaining({ page: 10, current: false })
		);

		expect(paginate(55, 5).slice(-1)[0]).toEqual(
			expect.objectContaining({ page: 55, current: false })
		);

		expect(paginate(123, 5).slice(-1)[0]).toEqual(
			expect.objectContaining({ page: 123, current: false })
		);
	});
	...
	describe("Ellipsis", () => {
		...
		describe("ellipsis on both sides", () => {
			it("appears when current active is higher than 4 and lower than pages - 3", () => {
				...
				expect(paginate(10, 8).slice(-2)[0]).not.toEqual(
					expect.objectContaining({ page: null })
				);
			});
		});
	...
	describe("Error throws", () => {
		it("throws error when currentActive is greater than total pages", () => {
			expect(() => paginate(10, 11)).toThrowError(
				"CurrentActive can not be greater than total pages."
			);
		});

		it("throws error when currentActive is 0 or less", () => {
			expect(() => paginate(10, 0)).toThrowError(
				"CurrentActive must be greater than 0"
			);
			expect(() => paginate(10, -1)).toThrowError(
				"CurrentActive must be greater than 0"
			);
		});
	});
